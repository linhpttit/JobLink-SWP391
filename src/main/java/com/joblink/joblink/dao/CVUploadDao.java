...
public class CVUploadDao {
    ...
    public List<CVUpload> findRecentBySeekerId(int seekerId, int limit) {
        String sql = """
            SELECT TOP (?) cv_id, seeker_id, full_name, phone_number, email,
                   preferred_location, years_of_experience, current_job_level,
                   work_mode, expected_salary, current_salary, cover_letter,
                   cv_file_url, cv_file_name, uploaded_at
            FROM CVUpload
            WHERE seeker_id = ?
    ...
    public CVUpload findById(int cvId) {
        String sql = """
            SELECT cv_id, seeker_id, full_name, phone_number, email,
                   preferred_location, years_of_experience, current_job_level,
                   work_mode, expected_salary, current_salary, cover_letter,
                   cv_file_url, cv_file_name, uploaded_at
            FROM CVUpload
