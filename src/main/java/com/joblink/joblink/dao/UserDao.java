...
public class UserDao {
    ...
    public UserDao(JdbcTemplate jdbc, PasswordEncoder passwordEncoder) {
        this.jdbc = jdbc;
    ...
    public User login(String email, String rawPassword) {
        ...
        String sql = """
                SELECT user_id, email, role, username, url_avt, created_at, password_hash
                FROM dbo.Users
                WHERE email = ?
                """;
        try {
    ...
    public int resetPassword(String email, String rawPassword) {
        ...
        try {
            return jdbc.update(sql, hashedPassword, email.trim());
        } catch (Exception e) {
            System.err.println("[UserDao] Reset password error: " + e.getMessage());
            return 0;
        }
    }
