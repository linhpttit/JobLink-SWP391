...
public class AdminController {
    ...
    public String employer(
        ...
        List<Map<String, Object>> employerList = employers.stream()
            .map(emp -> {
                ...
                // Xác định trạng thái dựa trên trường approved (bỏ logic tự động duyệt sau 7 ngày)
                Boolean approved = emp.getApproved();
                if (approved == null) {
                    // Chưa được xét duyệt
                    ...
                    map.put("statusText", "Chấp nhận");
                } else {
                    // Đã bị từ chối
                    map.put("status", "rejected");
                    map.put("statusText", "Từ chối");
                }
    ...
    @PostMapping("/employer/{userId}/update-status")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> updateEmployerStatus(
        ...
        try {
            ...
            if (employer == null) {
                System.err.println("❌ Không tìm thấy employer với user_id: " + userId);
                response.put("success", false);
                response.put("message", "Không tìm thấy nhà tuyển dụng");
            ...
            if ("pending".equals(status)) {
                employer.setApproved(null);
            } else if ("accepted".equals(status)) {
                employer.setApproved(true);
                isAccepted = true;
                shouldSendEmail = true;
                // Khi chấp nhận, đảm bảo user được enabled
                if (employer.getUser() != null) {
                    employer.getUser().setEnabled(true);
                ...
                isAccepted = false;
                shouldSendEmail = true;
                // Khi từ chối, khóa user
                if (employer.getUser() != null) {
                    employer.getUser().setEnabled(false);
                    userRepository.save(employer.getUser());
                }
            }

            employerRepository.save(employer);
            System.out.println("✅ Đã cập nhật trạng thái employer với user_id: " + userId + " -> " + status);
            
            // Gửi email thông báo tự động nếu được chấp nhận hoặc từ chối
            if (shouldSendEmail && employer.getUser() != null && employer.getUser().getEmail() != null) {
                String email = employer.getUser().getEmail();
                String companyName = employer.getCompanyName() != null ? employer.getCompanyName() : email;
                emailService.sendEmployerStatusNotification(email, companyName, isAccepted);
            }
            ...
            System.err.println("❌ Lỗi khi cập nhật trạng thái employer: " + e.getMessage());
            e.printStackTrace();
            response.put("success", false);
    ...
    /**
     * Endpoint để tạo EmployerProfile cho các employer đã đăng ký nhưng chưa có profile
     * (Sử dụng để fix dữ liệu cũ)
     */
    @PostMapping("/employer/create-missing-profiles")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> createMissingEmployerProfiles(HttpSession session) {
        Map<String, Object> response = new java.util.HashMap<>();
        ...
        try {
            // Lấy tất cả users có role = employer
            List<com.joblink.joblink.entity.User> employerUsers = userRepository.findAll()
                .stream()
            ...
            for (com.joblink.joblink.entity.User user : employerUsers) {
                Integer userId = user.getUserId();
                
                // Kiểm tra xem đã có EmployerProfile chưa
                ...
                // Tạo EmployerProfile mới
                Employer newEmployer = new Employer();
                newEmployer.setUser(user);
                
                String defaultCompanyName = user.getEmail() != null ? user.getEmail().split("@")[0] : "Công ty mới";
                ...
                newEmployer.setApproved(null); // Chờ xét duyệt

                employerRepository.save(newEmployer);
                createdCount++;
                System.out.println("✅ Đã tạo EmployerProfile cho user_id: " + userId);
            }

            response.put("success", true);
            response.put("message", "Đã tạo " + createdCount + " EmployerProfile mới, " + existingCount + " đã tồn tại");
            response.put("created", createdCount);
            response.put("existing", existingCount);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            System.err.println("❌ Lỗi khi tạo EmployerProfile: " + e.getMessage());
            e.printStackTrace();
            response.put("success", false);
            response.put("message", "Lỗi server: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
    ...
    /**
     * Endpoint để tạo JobSeekerProfile cho các job seeker đã đăng ký nhưng chưa có profile
     * (Sử dụng để fix dữ liệu cũ)
     */
    @PostMapping("/jobseeker/create-missing-profiles")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> createMissingJobSeekerProfiles(HttpSession session) {
        Map<String, Object> response = new java.util.HashMap<>();
        ...
        try {
            // Lấy tất cả users có role = seeker
            List<com.joblink.joblink.entity.User> seekerUsers = userRepository.findAll()
                .stream()
                .filter(u -> "seeker".equalsIgnoreCase(u.getRole()))
                .toList();

            int createdCount = 0;
            int existingCount = 0;

            for (com.joblink.joblink.entity.User user : seekerUsers) {
                Integer userId = user.getUserId();
                
                // Kiểm tra xem đã có JobSeekerProfile chưa
                Optional<JobSeekerProfile> existingProfile = jobSeekerRepo.findByUserId(userId);
                if (existingProfile.isPresent()) {
                    existingCount++;
                    continue;
                }

                // Tạo JobSeekerProfile mới với thông tin mặc định
                JobSeekerProfile newProfile = new JobSeekerProfile();
                newProfile.setUserId(userId);
                newProfile.setEmail(user.getEmail());
                newProfile.setFullName(null);
                newProfile.setGender(null);
                newProfile.setLocation(null);
                newProfile.setHeadline(null);
                newProfile.setExperienceYears(null);
                newProfile.setAbout(null);
                newProfile.setPhone(null);
                newProfile.setDateOfBirth(null);
                newProfile.setAvatarUrl(null);
                newProfile.setCompletionPercentage(0);
                newProfile.setReceiveInvitations(true); // Mặc định cho phép nhận lời mời
                newProfile.setIsLocked(false); // Mặc định không khóa
                newProfile.setUpdatedAt(LocalDateTime.now());

                jobSeekerRepo.save(newProfile);
                createdCount++;
                System.out.println("✅ Đã tạo JobSeekerProfile cho user_id: " + userId);
            }

            response.put("success", true);
            response.put("message", "Đã tạo " + createdCount + " JobSeekerProfile mới, " + existingCount + " đã tồn tại");
            response.put("created", createdCount);
            response.put("existing", existingCount);
            return ResponseEntity.ok(response);
        } catch (Exception e) {
            System.err.println("❌ Lỗi khi tạo JobSeekerProfile: " + e.getMessage());
            e.printStackTrace();
            response.put("success", false);
            response.put("message", "Lỗi server: " + e.getMessage());
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(response);
