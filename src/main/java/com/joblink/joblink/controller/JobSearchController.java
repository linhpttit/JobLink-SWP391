...
public class JobSearchController {
    ...
    public ResponseEntity<Map<String, Object>> searchComprehensive(
        ...
        Map<String, Object> results = (Map<String, Object>) (Map<String, Object>) jobSearchService.searchJobsByComprehensiveFilters(
                keyword, provinceId, districtId, categoryId, workType,
    ...
    @GetMapping("/api/jobs-by-skills")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> getJobsBySkills(
            @RequestParam List<Integer> skillIds,
            @RequestParam(required = false) Integer provinceId,
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "5") int pageSize) {

        int pId = provinceId != null ? provinceId : 0;
        List<Map<String, Object>> jobs = jobSearchService.getJobsBySkills(skillIds, pId);

        Map<String, Object> result = new HashMap<>();
        result.put("jobs", jobs);
        result.put("total", jobs.size());
        result.put("totalPages", Math.ceil((double) jobs.size() / pageSize));

        return ResponseEntity.ok(result);
    }

    @GetMapping("/api/jobs-by-company/{companyId}")
    @ResponseBody
    public ResponseEntity<List<Map<String, Object>>> getJobsByCompany(@PathVariable int companyId) {
        List<Map<String, Object>> jobs = jobSearchService.getJobsByCompany(companyId);
        return ResponseEntity.ok(jobs);
    }

    @PostMapping("/job-detail/{jobId}")
    public String applyForJob(@PathVariable int jobId, HttpSession session) {
        User user = (User) session.getAttribute("user");
        if (user == null) return "redirect:/signin";

        return "redirect:/jobseeker/job-detail/" + jobId;
    }
}
