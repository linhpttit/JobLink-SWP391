...
public class PayOSPaymentController {
    ...
    private static final Logger logger = Logger.getLogger(PayOSPaymentController.class.getName());
    ...
    public String showPackagesPage(HttpSession session, Model model) {
        try {
            ...
            // 1. Lấy tất cả các gói để hiển thị
            List<PremiumPackages> packages = packageRepository.findByUserTypeAndIsActiveTrueOrderByPriceAsc("JOBSEEKER");
            model.addAttribute("packages", packages);

            // 2. Lấy gói đăng ký HIỆN TẠI và chi tiết gói ĐANG HOẠT ĐỘNG
            PremiumSubscriptions currentSub = null;
            PremiumPackages activePackage = null; // <-- BIẾN MỚI QUAN TRỌNG

            if (seeker != null) {
                currentSub = subscriptionService.getCurrentActiveSubscription(seeker.getSeekerId());
            }

            // 3. Nếu người dùng có gói đang hoạt động, hãy tìm chi tiết của gói đó
            if (currentSub != null && currentSub.getIsActive()) {
                // Dùng packageRepository đã có để tìm gói theo packageId từ currentSub
                activePackage = packageRepository.findById(currentSub.getPackageId())
                        .orElse(null); // Sẽ trả về null nếu gói bị xóa
            }

            model.addAttribute("currentSubscription", currentSub);
            model.addAttribute("activePackage", activePackage); // <-- GỬI GÓI ĐANG HOẠT ĐỘNG RA VIEW
