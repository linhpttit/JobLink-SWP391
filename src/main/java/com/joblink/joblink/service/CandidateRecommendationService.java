...
public class CandidateRecommendationService {
     ...
     */
    private List<JobSeekerProfile2> getAllJobSeekers() {
        try {
            String sql = """
                SELECT 
                    jsp.seeker_id,
            ...
            return jdbcTemplate.query(sql, (rs, rowNum) -> {
                JobSeekerProfile2 profile = new JobSeekerProfile2();
     ...
     */
    private List<JobSeekerProfile2> getAvailableJobSeekers(Long jobId) {
        try {
            String sql = """
                SELECT 
                    jsp.seeker_id,
     ...
     */
    private CandidateRecommendationDTO calculateMatch(JobSeekerProfile2 seeker,
                                                      Set<String> jobSkills,
                                                      Integer minExperience) {
        int seekerId = seeker.getSeekerId();

        // 1. Lấy skills của ứng viên
        List<Skill2> seekerSkills = skillDao.findBySeekerId(seekerId);
