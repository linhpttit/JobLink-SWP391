...
public class PaymentService implements IPaymentService {
    ...
    public PaymentResponse createPayment(JobSeekerProfile seeker, PremiumPackages pkg) throws Exception {
        ...
        // 2. Tạo Invoice PENDING
        Invoice invoice = Invoice.builder()
                .userId(seeker.getUserId())
                .seekerId(seeker.getSeekerId())
                .subscriptionId(savedSub.getSubscriptionId())
                .amount(pkg.getPrice())
                .status("PENDING")
                .issuedAt(LocalDateTime.now())
                .dueAt(LocalDateTime.now().plusHours(1)) // Hóa đơn hết hạn sau 1h
                .build();
        Invoice savedInvoice = invoiceRepository.save(invoice);

        // 3. Tạo mã giao dịch (orderId)
        // Dùng cú pháp "PREMIUM_[InvoiceID]" để nội dung chuyển khoản dễ nhận diện
        String orderId = "PREMIUM" + savedInvoice.getInvoiceId();
        savedInvoice.setTxnRef(orderId);
        invoiceRepository.save(savedInvoice);

        // 4. Tạo Payment PENDING
        Payment payment = new Payment();
        payment.setInvoiceId(savedInvoice.getInvoiceId());
        payment.setProvider("BANK_TRANSFER"); // Đánh dấu là chuyển khoản
        ...
        // 6. Trả về thông tin cho Controller
    ...
    public Invoice getInvoiceByOrderId(String orderId) {
        // Tìm Payment trước, rồi lấy invoiceId từ payment
        Payment payment = getPaymentByOrderId(orderId);
        return invoiceRepository.findById(payment.getInvoiceId())
                .orElseThrow(() -> new RuntimeException("Không tìm thấy Invoice cho Payment: " + orderId));
    }
