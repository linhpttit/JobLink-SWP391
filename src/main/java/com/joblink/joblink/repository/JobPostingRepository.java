...
public interface JobPostingRepository extends JpaRepository<JobPosting, Long> {
    ...
    List<JobPosting> findByEmployerIdOrderByPostedAtDesc(Long id);
    
    long countByEmployerIdAndStatusIgnoreCase(Long employerId, String status);

    List<JobPosting> findByEmployerIdAndSubmissionDeadlineBetweenOrderBySubmissionDeadlineAsc(
            Long id, LocalDate from, LocalDate to
    );

	Page<JobPosting> findAll(Specification<JobPosting> spec, Pageable pageable);
