...
public class AuthController {
    ...
    public String doLogin(@ModelAttribute LoginRequest form,
        ...
        // 2. Kiểm tra nếu sai email/mật khẩu
        if (u == null) {
        ...
        // ==========================================================
        // ===> 3. THÊM BƯỚC KIỂM TRA TÀI KHOẢN BỊ KHÓA <===
        // ==========================================================
        // Giả sử model 'User' (com.joblink.joblink.auth.model.User)
        // của bạn có phương thức 'isActive()' trả về boolean.
        // (Nếu tên phương thức khác, ví dụ: 'getStatus()', bạn hãy điều chỉnh lại)

        if (!u.isEnabled()) {
            System.err.println("[AuthController] ⚠️ Login failed: Account is deactivated - " + u.getEmail());
            model.addAttribute("error", "Tài khoản này đã bị khóa hoặc vô hiệu hóa.");
            model.addAttribute("loginRequest", form); // Giữ lại email đã nhập
            return "signin"; // Trả về trang đăng nhập với thông báo lỗi
        }
        // ==========================================================


        // 4. Nếu mọi thứ OK -> Tạo session và tiếp tục
