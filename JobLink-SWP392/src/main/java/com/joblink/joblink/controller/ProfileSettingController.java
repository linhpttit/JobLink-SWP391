...
public class ProfileSettingController {
    ...
    public ResponseEntity<Map<String, Object>> deleteAccount(HttpSession session) {
        UserSessionDTO user = (UserSessionDTO) session.getAttribute("user");
        if (user == null) {
            // "User not authenticated" phù hợp hơn là "Không có quyền truy cập" trong ngữ cảnh này
            return ResponseEntity.status(401).body(Map.of("success", false, "message", "User not authenticated."));
        }

        try {
            int userId = user.getUserId();
            System.out.println("🗑️ Đang vô hiệu hóa (xóa mềm) tài khoản cho user ID: " + userId);

            // Sử dụng service đã có trong controller: accountService
            boolean success = accountService.deactivateAccount(userId);

            if (success) {
                System.out.println("✅ Đã khóa tài khoản ID: " + userId);
                session.invalidate(); // Xóa session sau khi vô hiệu hóa tài khoản
                return ResponseEntity.ok(Map.of("success", true, "message", "Đã khóa tài khoản thành công"));
            } else {
                System.out.println("❌ Không tìm thấy tài khoản để khóa, ID: " + userId);
                // Dùng 404 Not Found hoặc 400 Bad Request đều hợp lý
