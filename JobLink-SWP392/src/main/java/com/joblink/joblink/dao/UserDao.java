...
public class UserDao {
    ...
    public int setEnabled(int userId, boolean enabled) {
        // Cột 'enabled' trong DB phải là kiểu BIT (0 hoặc 1) hoặc INT
        String sql = "UPDATE dbo.Users SET enabled = ? WHERE user_id = ?";
        try {
            // enabled = true -> 1, enabled = false -> 0
            return jdbc.update(sql, enabled ? 1 : 0, userId);
        } catch (Exception e) {
            System.err.println("[UserDao] setEnabled SQL error: " + e.getMessage());
            e.printStackTrace();
            return 0;
        }
    }
